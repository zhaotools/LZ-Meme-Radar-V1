import type { SecurityFacts } from "../src/shared/types";
...
export function normalizeGoPlus(raw: GoPlusToken | undefined, pairAddress?: string | null): SecurityFacts {
  ...
  return {
    ...
    lpLockedPct: number(raw.lp_holders ? (raw.lp_holders as Array<Record<string, unknown>>)
  ...
  for (let index = 0; index < unique.length; index += 20) {
    ...
    try {
      ...
      batch.forEach((address) => {
        const raw = body.result?.[address] ?? body.result?.[address.toLowerCase()];
        results.set(address, {
      ...
      batch.forEach((address) => results.set(address, { facts: {}, holderCount: null }));
    }
  }
  return results;
}
